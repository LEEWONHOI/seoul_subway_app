...
import com.example.wonhoi_seoul_subway_info_app.databinding.ActivityMainBinding
...
import com.example.wonhoi_seoul_subway_info_app.presentation.stationarrivals.StationArrivalsFragmentArgs
    ...
    private fun bindViews() {
        navigationController.addOnDestinationChangedListener { _, destination, argument ->
            if (destination.id == R.id.station_arrivals_dest) { // 화면 전환용 체크
                ...
                title = StationArrivalsFragmentArgs.fromBundle(argument!!).station.name     // 타입을 알 수 있는 argument 로 받은 뒤 station 정보 가져오기
